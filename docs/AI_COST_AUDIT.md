...
- FastAPI generation: onboarding, intake chat, call prep, and outreach run through `services/api/app/ai/prompts.py`.
...
`services/api/app/ai/prompts.py` now retries `_generate_json` once when parsing fails because the response is not a JSON object. It does not retry timeouts, missing API keys, provider failures, or other non-format errors.
