...
export function buildExtractKickoffIdeaMessages(userMessage: string): AIMessage[] {
  return [
    {
      role: 'user',
      content: `A founder has just described their startup idea. Extract an in depth summary and assess quality.
...
Rules:
- ideaSummary must be in depth, written as a neutral description (not first-person)
