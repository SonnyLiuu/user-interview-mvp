...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const [updated] = await db
    .update(people)
    .set({ board_status: 'scheduled', call_scheduled_at: scheduledAt, expires_at: null, updated_at: new Date() })
  ...
  await db.insert(person_events).values({
    person_id: personId,
    type: 'stage_changed',
    metadata: matchEventMetadata(current, { to: 'scheduled', scheduled_at: scheduledAt.toISOString() }, 3),
  });
